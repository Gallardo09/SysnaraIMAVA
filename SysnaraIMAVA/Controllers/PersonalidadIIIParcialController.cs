...
{
    public class PersonalidadIiiParcialController : Controller // III Parcial
    {
        private readonly DbimavaContext _context;

        public PersonalidadIiiParcialController(DbimavaContext context) // III Parcial
        {
            _context = context;
        }

        // GET: PersonalidadIiParcialController // III Parcial
        ...
        {
            var query = _context.PersonalidadIiis
        ...
        public IActionResult VerificarDatos(int? idAño, string idGrado)
        {
            ...
            var datosFiltrados = _context.PersonalidadIiis // III Parcial
            ...
            try
            {
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    for (int i = 0; i < datosFiltrados.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 8, 9].Value = personalidad.IdpersonalidadIii; // III Parcial ID
                    ...
                    var fileName = $"Personalidad_{idAño}_{idGrado}_III.xlsx"; // Nombre dinámico
        ...
        public IActionResult DescargarExcelenBlancoPersonalidadII(int? idAño, string idGrado)
        {
            ...
            try
            {
                // Obtener los datos filtrados para III Parcial (cambiamos 'IIParcial' por 'IIIParcial')
                var datosFiltrados = _context.PersonalidadIiis //IIIParcial
                                              .Where(p => p.Idaño == idAño && p.Idgrado == idGrado && p.IdpersonalidadIii != null) //II Parcial ID
                                              .ToList();
                ...
                "Inasistencias", "Sistema", "Sistema Clase", "Sistema Tiempo"
                    ...
                    else
                    {
                        ...
                        foreach (var dato in datosFiltrados)
                        {
                            ...
                            worksheet.Cells[row, 9].Value = dato.IdpersonalidadIii; // III Parcial ID
        ...
        public async Task<IActionResult> SubirPersonalidades(IFormFile archivoExcel)
        {
            ...
            try
            {
                using (var package = new ExcelPackage(archivoExcel.OpenReadStream()))
                {
                    ...
                    List<PersonalidadIii> personalidades = new List<PersonalidadIii>(); //III Parcial

                    for (int row = 8; row <= rowCount; row++) // Comienza en la fila 8 (por la cabecera)
                    {
                        // Leer los datos de cada fila
                        var personalidad = new PersonalidadIii //III Parcial
                        {
                            ...
                            IdpersonalidadIii = int.TryParse(worksheet.Cells[row, 9].Text, out var idPersonalidad) ? idPersonalidad : 0, //IIIParcial
                        ...
                        if (tipoIngreso == "NUEVOINGRESO")
                        {
                            ...
                            var existingRecord = _context.PersonalidadIiis //IIIParcial
                            ...
                            _context.PersonalidadIiis.Add(personalidad); //IIIParcial
                        }
                        else if (tipoIngreso == "EDITANDO")
                        {
                            // Buscar y actualizar el registro existente
                            var personalidadExistente = _context.PersonalidadIiis //IIIParcial
                                .FirstOrDefault(p => p.Idest == personalidad.Idest && p.Idaño == personalidad.Idaño && p.IdpersonalidadIii == personalidad.IdpersonalidadIii); //IIParcial ID
                            ...
                            if (personalidadExistente != null)
                            {
                                ...
                                _context.PersonalidadIiis.Update(personalidadExistente); //IIParcial
                            }
                            else
                            {
