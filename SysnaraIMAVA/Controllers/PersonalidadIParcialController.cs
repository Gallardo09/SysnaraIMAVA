...
{
    public class PersonalidadIParcialController : Controller //IParcial
    {
        private readonly DbimavaContext _context;

        public PersonalidadIParcialController(DbimavaContext context) //IParcial
        {
            _context = context;
        }


        // GET: PersonalidadIParcialController //IParcial
        ...
        {
            var query = _context.Personalidads
            ...
            {
                return Json(new { success = false, message = "Ops, no hay datos de personalidad registrados en este Grado y I Parcial." });
            }
        ...
        public IActionResult VerificarDatos(int? idAño, string idGrado)
        {
            ...
            var datosFiltrados = _context.Personalidads //IParcial
        ...
        public IActionResult DescargarExcel(int? idAño, string idGrado)
        {
            ...
            try
            {
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    headerCell.Value = "PERSONALIDAD ESTUDIANTIL - I PARCIAL";
                    ...
                    worksheet.Cells[7, 13].Value = "Espíritu de Trabaj7+0o";
                    ...
                    for (int i = 0; i < datosFiltrados.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 8, 9].Value = personalidad.Idpersonalidad; //IParcial ID
                    ...
                    var fileName = $"Personalidad_{idAño}_{idGrado}.xlsx"; // Nombre dinámico
        ...
        public IActionResult DescargarExcelenBlancoPersonalidad(int? idAño, string idGrado)
        {
            if (idAño == null || string.IsNullOrEmpty(idGrado))
            {
                return BadRequest("Debe proporcionar ambos parámetros: Año y Grado.");
            ...
            try
            {
                // Obtener los datos filtrados
                var datosFiltrados = _context.Personalidads //IParcial
                                              .Where(p => p.Idaño == idAño && p.Idgrado == idGrado && p.Idpersonalidad != null) //IParcial ID
                                              .ToList();
                ...
                "Inasistencias", "Sistema", "Sistema Clase", "Sistema Tiempo"
                    ...
                    else
                    {
                        ...
                        foreach (var dato in datosFiltrados)
                        {
                            ...
                            worksheet.Cells[row, 9].Value = dato.Idpersonalidad; //IParcial ID
                    ...
                    var fileName = $"PersonalidadNueva_{idAño}_{idGrado}.xlsx";
        ...
        public async Task<IActionResult> SubirPersonalidades(IFormFile archivoExcel)
        {
            ...
            try
            {
                using (var package = new ExcelPackage(archivoExcel.OpenReadStream()))
                {
                    ...
                    List<Personalidad> personalidades = new List<Personalidad>();

                    for (int row = 8; row <= rowCount; row++) // Comienza en la fila 8 (por la cabecera)
                    {
                        // Leer los datos de cada fila
                        var personalidad = new Personalidad
                        {
                            ...
                            Idpersonalidad = int.TryParse(worksheet.Cells[row, 9].Text, out var idPersonalidad) ? idPersonalidad : 0, //IParcial
                        ...
                        if (tipoIngreso == "NUEVOINGRESO")
                        {
                            ...
                            var existingRecord = _context.Personalidads //IParcial
                            ...
                            _context.Personalidads.Add(personalidad); //IParcial
                        }
                        else if (tipoIngreso == "EDITANDO")
                        {
                            // Buscar y actualizar el registro existente
                            var personalidadExistente = _context.Personalidads //IParcial
                                .FirstOrDefault(p => p.Idest == personalidad.Idest && p.Idaño == personalidad.Idaño && p.Idpersonalidad == personalidad.Idpersonalidad); //IParcial ID
                            ...
                            if (personalidadExistente != null)
                            {
                                ...
                                _context.Personalidads.Update(personalidadExistente); //IParcial
                            }
                            else
                            {
