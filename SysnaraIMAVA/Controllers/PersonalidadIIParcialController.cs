...
{
    public class PersonalidadIiParcialController : Controller // II Parcial
    {
        private readonly DbimavaContext _context;

        public PersonalidadIiParcialController(DbimavaContext context) // II Parcial
        {
            _context = context;
        }

        // GET: PersonalidadIiParcialController // II Parcial
        ...
        {
            var query = _context.PersonalidadIis
            ...
            var personalidades = await query
                .Select(m => new {
                    ...
                    m.Genero,
                    idpersonalidadii = m.IdpersonalidadIi,
                    m.Puntualidad,
            ...
            {
                return Json(new { success = false, message = "Ops, no hay datos de personalidad registrados en este Grado y II Parcial." });
            }
        ...
        public IActionResult VerificarDatos(int? idAño, string idGrado)
        {
            ...
            var datosFiltrados = _context.PersonalidadIis // II Parcial
        ...
        public IActionResult DescargarExcel(int? idAño, string idGrado)
    {
        ...
        try
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                headerCell.Value = "PERSONALIDAD ESTUDIANTIL - II PARCIAL";
                ...
                for (int i = 0; i < datosFiltrados.Count; i++)
                {
                    ...
                    worksheet.Cells[i + 8, 9].Value = personalidad.IdpersonalidadIi; // II Parcial ID
                ...
                var fileName = $"Personalidad_{idAño}_{idGrado}_II.xlsx"; // Nombre dinámico
        ...
        public IActionResult DescargarExcelenBlancoPersonalidadII(int? idAño, string idGrado)
        {
            ...
            try
            {
                // Obtener los datos filtrados para II Parcial (cambiamos 'IParcial' por 'IIParcial')
                var datosFiltrados = _context.PersonalidadIis //IIParcial
                                              .Where(p => p.Idaño == idAño && p.Idgrado == idGrado && p.IdpersonalidadIi != null) //II Parcial ID
                                              .ToList();

                using (var package = new ExcelPackage())
                {
                    var worksheet = package.Workbook.Worksheets.Add("Personalidad II Parcial");
                ...
                "Inasistencias", "Sistema", "Sistema Clase", "Sistema Tiempo"
                    ...
                    else
                    {
                        ...
                        foreach (var dato in datosFiltrados)
                        {
                            ...
                            worksheet.Cells[row, 9].Value = dato.IdpersonalidadIi; // II Parcial ID
                    ...
                    var fileName = $"Personalidad_II_{idAño}_{idGrado}.xlsx";
        ...
        public async Task<IActionResult> SubirPersonalidades(IFormFile archivoExcel)
        {
            ...
            try
            {
                using (var package = new ExcelPackage(archivoExcel.OpenReadStream()))
                {
                    ...
                    List<PersonalidadIi> personalidades = new List<PersonalidadIi>();

                    for (int row = 8; row <= rowCount; row++) // Comienza en la fila 8 (por la cabecera)
                    {
                        // Leer los datos de cada fila
                        var personalidad = new PersonalidadIi
                        {
                            ...
                            IdpersonalidadIi = int.TryParse(worksheet.Cells[row, 9].Text, out var idPersonalidad) ? idPersonalidad : 0, //IIParcial
                        ...
                        if (tipoIngreso == "NUEVOINGRESO")
                        {
                            ...
                            var existingRecord = _context.PersonalidadIis //IIParcial
                            ...
                            _context.PersonalidadIis.Add(personalidad); //IIParcial
                        }
                        else if (tipoIngreso == "EDITANDO")
                        {
                            // Buscar y actualizar el registro existente
                            var personalidadExistente = _context.PersonalidadIis //IIParcial
                                .FirstOrDefault(p => p.Idest == personalidad.Idest && p.Idaño == personalidad.Idaño && p.IdpersonalidadIi == personalidad.IdpersonalidadIi); //IIParcial ID
                            ...
                            if (personalidadExistente != null)
                            {
                                ...
                                _context.PersonalidadIis.Update(personalidadExistente); //IIParcial
                            }
                            else
                            {
