...
{
    public class PersonalidadIipParcialController : Controller
    {
        private readonly DbimavaContext _context;

        public PersonalidadIipParcialController(DbimavaContext context)
        {
            _context = context;
        }

        // GET: PersonalidadIipParcialController
        ...
        {
            var query = _context.PersonalidadIips
        ...
        public IActionResult VerificarDatos(int? idAño, string idGrado)
        {
            ...
            var datosFiltrados = _context.PersonalidadIips
            ...
            try
            {
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    for (int i = 0; i < datosFiltrados.Count; i++)
                    {
                        ...
                        worksheet.Cells[i + 8, 9].Value = personalidad.IdpersonalidadIi;
                    ...
                    for (int i = 8; i < 8 + datosFiltrados.Count; i++)
                    {
                        ...
                        {
                            worksheet.Cells[i, col].Style.Locked = false;
                        }
                    }
        ...
        public IActionResult DescargarExcelenBlancoPersonalidadII(int? idAño, string idGrado)
        {
            ...
            {
                var datosFiltrados = _context.PersonalidadIips
                                              .Where(p => p.Idaño == idAño && p.Idgrado == idGrado && p.IdpersonalidadIi != null)
                                              .ToList();
        ...
        public async Task<IActionResult> SubirPersonalidades(IFormFile archivoExcel)
        {
            ...
            try
            {
                using (var package = new ExcelPackage(archivoExcel.OpenReadStream()))
                {
                    ...
                    List<PersonalidadIip> personalidades = new List<PersonalidadIip>();

                    for (int row = 8; row <= rowCount; row++)
                    {
                        var personalidad = new PersonalidadIip
                        {
                            ...
                            IdpersonalidadIi = int.TryParse(worksheet.Cells[row, 9].Text, out var idPersonalidad) ? idPersonalidad : 0,
                        ...
                        {
                            var existingRecord = _context.PersonalidadIips
                            ...
                            _context.PersonalidadIips.Add(personalidad);
                        }
                        else if (tipoIngreso == "EDITANDO")
                        {
                            var personalidadExistente = _context.PersonalidadIips
                                .FirstOrDefault(p => p.Idest == personalidad.Idest && p.Idaño == personalidad.Idaño && p.IdpersonalidadIi == personalidad.IdpersonalidadIi);
                            ...
                            if (personalidadExistente != null)
                            {
                                ...
                                _context.PersonalidadIips.Update(personalidadExistente);
                            }
                            else
                            {
